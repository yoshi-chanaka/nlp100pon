...
"""
nvidia-smi
nohup python -u nlp100pon_98_finetune.py >& ../materials/output_finetune.txt &
"""
...
if __name__ == "__main__":
    ...
    batch_size = 128
    dataset, vocab_src, vocab_tgt = load_data(extension='kftt')
    ...
    pretrained_modelpath = '../models/model_pretrained.pth'
    model.load_state_dict(torch.load(pretrained_modelpath, map_location=device))
    ...
    writer = SummaryWriter(log_dir="../materials/logs_finetuned")
    NUM_EPOCHS = 15
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            model_dir_path.joinpath(f'model_finetuned_{str(epoch + 1).zfill(2)}epochs.pth'))

    writer.close()
    torch.save(model.module.state_dict(), model_dir_path.joinpath('model_finetuned.pth'))
    print(datetime.datetime.now())
