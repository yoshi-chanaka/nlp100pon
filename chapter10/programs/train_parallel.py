    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for src, tgt in tqdm(train_dataloader):
            ...
            pred = model(
                src=src_tr,
                trg=tgt_input_tr,
                src_mask=src_mask.repeat(src_tr.shape[0], 1, 1), # (バッチサイズ, マスクサイズ, マスクサイズ)
                tgt_mask=tgt_mask.repeat(src_tr.shape[0], 1, 1), # (バッチサイズ, マスクサイズ, マスクサイズ)
                src_padding_mask=src_padding_mask,
                tgt_padding_mask=tgt_padding_mask,
                memory_key_padding_mask=src_padding_mask
            )
            pred = pred.transpose(0, 1) # (バッチサイズ, 系列長, 語彙数) -> # (系列長, バッチサイズ, 語彙数)
