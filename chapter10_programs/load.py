...
"""
データの読み込み. load_data()でデータセットとvocabがとれる
"""
...
def load_data(max_len=150, only_vocab=False):
    ...
    for kind in ['train', 'dev', 'test']:
        corpus_src[kind] = load_corpus(f'../data/{kind}_ja.kftt')
        corpus_tgt[kind] = load_corpus(f'../data/{kind}_en.kftt')
    ...
    for kind in ['train', 'dev', 'test']:
        ...
        # srcset = pad_sequence(srcset, batch_first=True, padding_value=1)
        # tgtset = pad_sequence(tgtset, batch_first=True, padding_value=1)
        src_size = len(srcset)
        set_all = pad_sequence(srcset + tgtset, batch_first=True, padding_value=1)
